...
def main():
    ...
    with open(file_name, 'w') as f:
        ...
        f.write('<div  style="margin: 10px">' + 'Deposit report (' + group_mask+') from ' + start_date + ' to ' +
                finish_date + '</div>' + '\n')
...
while True:
    if __name__ == "__main__":
        if datetime.datetime.now().strftime('%H:%M:%S') == '07:00:05':
            main()
            print("Deposit report отправлен -", datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            time.sleep(10)
